...
export class Table {
  ...
  getHeadHeight() {
    return this.head.reduce(
      (acc, row) => acc + row.getMaxCellHeight(this.columns),
      0
    );
  }
  ...
  getHeight() {
    return (
      (this.settings.showHead ? this.getHeadHeight() : 0) + this.getBodyHeight()
    );
  }
}
...
function getStringWidth(cell: Cell, fontKitFont: FontKitFont) {
  ...
  const widestLineWidth = textArr
    .map((text) =>
      widthOfTextAtSize(text, fontKitFont, fontSize, characterSpacing)
    )
