...
const calcResizedHeadWidthPercentages = (arg: {
  ...
  const originalNextWidthPercentage =
    headWidthPercentages[changedHeadIndex + 1] ?? 0;
  const adjustment =
    headWidthPercentages[changedHeadIndex] - changedWidthPercentage;
  headWidthPercentages[changedHeadIndex] = changedWidthPercentage;
  if (changedHeadIndex + 1 < headWidthPercentages.length) {
    headWidthPercentages[changedHeadIndex + 1] =
      originalNextWidthPercentage + adjustment;
  }
...
const setBorder = (
  ...
  div.style[`border${borderPosition}`] = `${String(
    arg.schema.tableStyles.borderWidth
  )}mm solid ${arg.schema.tableStyles.borderColor}`;
};
...
const renderRowUi = (args: {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    Object.values(cells).forEach((cell, colIndex) => {
      ...
      if (arg.mode === "form") {
        mode =
          section === "body" && isEditing && !arg.schema.readOnly
            ? "designer"
            : "viewer";
...
export const uiRender = async (arg: UIRenderProps<TableSchema>) => {
  ...
  if (showHead) {
    renderRowUi({
      ...
      onChangeEditingPosition: (p) =>
        handleChangeEditingPosition(p, headEditingPosition),
    });
  }
  ...
  const createRemoveRowButtons = () => {
    ...
    return table.body.map((row, i) => {
      ...
      const removeRowButton = createButton({
        ...
        text: "-",
        onClick: () => {
          const newTableBody = body.filter(
  ...
  if (mode === "form" && onChange && !schema.readOnly) {
    ...
    createRemoveRowButtons().forEach((button) =>
  ...
  if (mode === "designer" && onChange) {
    const addColumnButton = createButton({
      ...
      onClick: (e) => {
        ...
        const scalingRatio =
          (100 - newColumnWidthPercentage) / totalCurrentWidth;
        const scaledWidths = schema.headWidthPercentages.map(
        ...
        onChange([
          ...
          },
          {
            ...
            value: JSON.stringify(
              bodyWidthRange.map((row, i) => row.concat(`Row ${i + 1}`))
            ),
          },
        ]);
      },
    ...
    table.columns.forEach((column, i, columns) => {
        ...
        onClick: () => {
          ...
          onChange([
            ...
            },
            {
              key: "content",
              value: JSON.stringify(
                bodyWidthRange.map((row) => row.filter((_, j) => j !== i))
              ),
            },
          ]);
        },
      ...
      const nextColumnRight =
        offsetX - px2mm(lineWidth) + table.columns[i + 1].width;
