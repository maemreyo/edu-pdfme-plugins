  ...
  const plugin: Plugin<DateSchema> = {
    ui: async (arg) => {
      ...
      const locale = getAirDatepickerLocale(
        schema.locale || options.lang || defaultLocale
      );
      ...
      if (type !== "date") {
        adButtons.push({
          ...
          onClick: (datepicker) => {
            datepicker.hide();
            const date = datepicker.selectedDates.length
    ...
    propPanel: {
      schema: ({ options, i18n, activeSchema, changeSchemas }) => {
        ...
        const locale = getAirDatepickerLocale(
          (activeSchema as { locale?: string }).locale ||
            options.lang ||
            defaultLocale
        );
