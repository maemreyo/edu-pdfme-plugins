...
export const uiRender = async (arg: UIRenderProps<MultiVariableTextSchema>) => {
  ...
  await parentUiRender({
    ...
    rootElement,
    onChange: (
      arg: { key: string; value: unknown } | { key: string; value: unknown }[]
    ) => {
      if (!Array.isArray(arg)) {
        ...
        throw new Error(
          "onChange is not an array, the parent text plugin has changed..."
        );
      }
    },
    ...rest,
  ...
  if (!textBlock) {
    throw new Error(
      'Text block not found. Ensure the text block has an id of "text-" + schema.id'
    );
  }
